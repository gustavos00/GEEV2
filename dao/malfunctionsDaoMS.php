...
class malfunctionsDAOMS implements malfunctionsDAO
{
    ...
    public function getAllWithFilter($f) {
        ...
        $sql = $this->pdo->prepare("SELECT avarias.*, prestadorservicos.* FROM avarias 
        LEFT JOIN prestadorservicos ON prestadorservicos_idprestadorservico = prestadorservicos.idprestadorservico" . $f);
        $sql->execute();
